...
class Monitor:
            ...
            if phi > 0:
                ...
                try:
                    ...
                    if sol['status'] == 'optimal':
                        u_opt = sol['x'][0]
                        u_opt = (v ** 2 - u_opt ** 2) / (4 * resistance) # Eq. 44
                except:
    ...
    def solve_qp(self, org_a, env):
        ...
        p_ice = min(max(env.model.pENGFBK.value + org_a[0], 0.0),
...
class GDSAC(object):
    def __init__(self, state_dim, action_dim, max_action, device, Logger, use_actor_tag=False, actor_hidden_width = [500, 400, 300],
                 critic_hidden_width = [500, 400, 300], batch_size=120, GAMMA=0.99, TAU = 0.005, alphalr = 5e-5,
        ...
        self.actor_hidden_width = actor_hidden_width
        self.critic_hidden_width = critic_hidden_width
    ...
    def init_networks(self):
        self.actor = Actor(self.state_dim, self.action_dim, self.actor_hidden_width, self.max_action).to(self.device)
        self.actor_target = copy.deepcopy(self.actor).to(self.device)
        self.actor_optimizer = torch.optim.Adam(self.actor.parameters(), lr=self.alr)
        print(self.actor)

        self.reward_critic1 = Critic(self.state_dim, self.action_dim, self.critic_hidden_width, -8, 10).to(self.device)
        ...
        self.reward_critic2 = Critic(self.state_dim, self.action_dim, self.critic_hidden_width, -8, 10).to(self.device)
...
if __name__ == '__main__':
    ...
    for i in range(len(seed_list)):
        ...
        kwargs = {
            ...
            "Logger": logger,
            "use_actor_tag": True,
            "actor_hidden_width": [300, 200, 100],
            "critic_hidden_width": [400, 400, 300],
            "batch_size": 120,
            "GAMMA": 0.99,
            "TAU": 0.005,
            "alphalr": 5e-5,
